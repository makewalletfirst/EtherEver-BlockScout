defmodule Indexer.Transform.TokenTransfers do
  ...
  # [HACK] 불량 토큰 데이터 무시를 위한 Wrapper 함수
  # 어떤 데이터가 들어오든 일단 받아서 내부 로직(do_parse_params)을 실행해보고,
  # 에러가 터지면 죽지 말고 nil(무시)을 리턴합니다.
  defp parse_params(log) do
    try do
      do_parse_params(log)
    rescue
      # 어떤 에러가 나더라도 무시합니다.
  ...
  defp do_parse_params(%{second_topic: second_topic, third_topic: third_topic, fourth_topic: nil} = log)
  ...
  defp do_parse_params(%{second_topic: second_topic, third_topic: nil, fourth_topic: nil} = log)
  ...
  defp do_parse_params(%{second_topic: second_topic, third_topic: third_topic, fourth_topic: fourth_topic} = log)
